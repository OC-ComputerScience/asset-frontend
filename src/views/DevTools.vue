...
const fetchDevUsersAndRoles = async () => {
  try {
    ...
    roleNames.value = rolesResponse.data
      .map((role) => role.name)
      .sort((a, b) => a.localeCompare(b)); // Sorting role names alphabetically
...
<template>
  <div>
    <v-container>
      <v-card class="elevation-1" style="max-width: 900px; margin: auto">
      ...
      <iframe
        ...
        src="https://open.spotify.com/embed/track/4BP3uh0hFLFRb5cjsgLqDh?utm_source=generator"
        width="100%"
        height="352"
        frameBorder="0"
        allowfullscreen=""
