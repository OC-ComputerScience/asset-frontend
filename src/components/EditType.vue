...
const retrieveFields = async () => {
  try {
    let response = await customFieldTypeServices.getAllForType(
      type.value.typeId
    );
...
const updateSequenceCount = (field) => {
  if (field.sequence >= 1) {
    fieldSequence.value = fieldSequence.value.filter(
      (num) => num !== field.sequence
    );
  }
...
const saveFields = async (typeId) => {
  ...
  try {
    for (let field of typeFields.value) {
      ...
      if (field.id) {
        response = await customFieldTypeServices.update(
          field.id,
          customFieldType
        );
      } else {
